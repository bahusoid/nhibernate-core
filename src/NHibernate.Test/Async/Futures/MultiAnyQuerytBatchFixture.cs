...
namespace NHibernate.Test.Criteria
{
	...
	public class MultiAnyQuerytBatchFixtureAsync : TestCaseMappingByCode
	{
		...
		[Test]
		public async Task CanCombineCriteriaAndHqlAsFutureAsync()
		{
		...
		[Test]
		public async Task CanCombineCriteriaAndHqlAsSepearateBatchAsync()
		{
			using (var session = OpenSession())
			{
		...
		[Test]
		public async Task TestMultiWithCollectionInitializedAsync()
		{
			using (var sqlLog = new SqlLogSpy())
			using (var session = OpenSession())
			{
		...
		[Test]
		public async Task TestMultiWithCollectionInitialized_AsFutureListAsync()
		{
			using (var sqlLog = new SqlLogSpy())
			using (var session = OpenSession())
			{
		...
		[Test, Explicit]
		public async Task TestQueryWithSubselectAsync()
		{
			using (var sqlLog = new SqlLogSpy())
			using (var session = OpenSession())
			{
				var ec = await (session.GetAsync<EntityEager>(_eagerId));
			}
		}
