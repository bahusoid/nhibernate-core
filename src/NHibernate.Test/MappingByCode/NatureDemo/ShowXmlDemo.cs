...
namespace NHibernate.Test.MappingByCode.NatureDemo
{
	...
	public class ShowXmlDemo
	{
		...
		{
			_allocated = ShowMemoryTaken("initial :");
			...
			foreach (var i in Enumerable.Range(0, 1000))
			{
				...
				{
					ShowMemoryTaken($"[{i}] Before disposing: ");
				}
				GC.Collect();

				ShowMemoryTaken($"[{i}] After disposing: ");
			}
			
		}

		private static long ShowMemoryTaken(string comment)
		{
			var totalMemory = GC.GetTotalMemory(true);
			Console.Error.WriteLine(comment + ((totalMemory - _allocated) / 1024.0).ToString("##.###") + "Kb");
			return totalMemory;
		}
