	...
	public partial class SqlGenerator : IErrorReporter
	{
		...
		private void EndFunctionTemplate(IASTNode m)
		{
			...
			else
			{
				...
				Out(methodNode.IsTransaperentCast() && methodNode.ExpectedType != null
					    ? ((CastFunction)template).Render(functionArguments.Args, methodNode.ExpectedType, sessionFactory)
					    : template.Render(functionArguments.Args, sessionFactory));
			}
		}
