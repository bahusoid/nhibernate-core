...
havingClause
	: ^(HAVING { HandleClauseStart( HAVING ); } logicalExpr)
	;
	finally { HandleClauseEnd( HAVING ); }
...
joinElement!
	...
	: ^(JOIN (j=joinType { SetImpliedJoinType($j.j); } )? (f=FETCH)? pRef=propertyRef (a=ALIAS)? (pf=propertyFetch)? (^((with=WITH) .*))? ) 
	{
		CreateFromJoinElement($pRef.tree,$a,$j.j,$f, $pf.tree, $with);
		SetImpliedJoinType(INNER);	// Reset the implied join type.
	}
	| ^(JOIN (j=joinType { SetImpliedJoinType($j.j); } )? q=query (a=ALIAS)? (^((with=WITH) .*))? ) 
	{
		CreateJoinSubquery($q.tree,$a,$j.j, $with);
		SetImpliedJoinType(INNER);	// Reset the implied join type.
	}
	;
	finally { HandleClauseEnd( JOIN ); }
