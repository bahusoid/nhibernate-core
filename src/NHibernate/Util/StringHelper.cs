...
namespace NHibernate.Util
{
	...
	public static class StringHelper
	{
		...
		internal static void Replace(StringBuilder template, string placeholder, string replacement, bool wholeWords)
		{
			bool IsWholeWord(string c) => WhiteSpace.Contains(c) || ClosedParen.Equals(c) || Comma.Equals(c);

			ReplaceByPredicate(template, placeholder, replacement, wholeWords ? (Predicate<string>) IsWholeWord : null);
		}
		...
		private static void ReplaceByPredicate(StringBuilder template, string placeholder, string replacement, Predicate<string> isWholeWord)
		{
			// sometimes a null value will get passed in here -> SqlWhereStrings are a good example
			...
			int startIndex = 0;
			int loc;
			while((loc = template.IndexOf(placeholder, startIndex)) >=0)
			{
				// NH different implementation (NH-1253)
				var indexAfterPlaceHolder = loc + placeholder.Length;
				if (isWholeWord != null && indexAfterPlaceHolder < template.Length)
				{
					string afterPlaceholder = template[indexAfterPlaceHolder].ToString();
					//After a token in HQL there can be whitespace, closedparen or comma.. 
					if (!isWholeWord(afterPlaceholder))
					{
						//If this is not a full token we don't want to touch it
						startIndex = indexAfterPlaceHolder;
						continue;
					}
				}
				template.Replace(placeholder, replacement, loc, placeholder.Length);
				startIndex = loc + replacement.Length;
			}
		}
